...
async function processDeposit(ctx, amount) {
  try {
    ...
    const response = await axios.get(
      `http://pymnt-xwan.gloryx.web.id/api/deposit?amount=${amount}&apikey=${API_KEY}`
    );
    ...
    for (const [transactionId, deposit] of pendingDeposits) {
      ...
      try {
        const res = await axios.get(
          `http://pymnt-xwan.gloryx.web.id/api/status/payment?transaction_id=${transactionId}&apikey=${API_KEY}`
        );
